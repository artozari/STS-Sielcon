...
function procesarVector(vector) {
  ...
  const { cantidades } = obtenerValoresDeNumerosIndividuales(vector, 4); // Obtener cantidades de números ganadores);
  const chi = chiSquaredConstantExpected(cantidades); // Chi cuadrado de los RPM
  const juegoIni = vector[0]?.gameNumber || null; // Número de juego inicial
  const juegoFin = vector[vector.length - 1]?.gameNumber || null; // Número de juego final
  ...
  const resultado = vector.map((item) => {
    ...
    const dato = {
      ...
      juegoIni: juegoIni, // Usar el valor calculado previamente
      juegoFin: juegoFin, // Usar el valor calculado previamente
      date: date,
    };
...
const obtenerValoresDeNumerosIndividuales = (vectorDeVectores, j) => {
  ...
  const ruleta = j === 4 ? ruletaEuropea : ruletaAmericana;
